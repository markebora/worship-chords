const VERSION='20260819-1130';
const CACHE='worship-chords-'+VERSION;
const ASSETS=[
  './',
  './index.html',
  './app.html?v=20260819-1003',
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('worship-chords-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()));
});

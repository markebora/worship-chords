...
function parseSongText(text){
 ...
 for(const raw of normalizeText(text).split('\n')){
  const line=raw.trim(); if(!line){flushPending();continue;}
  const h=line.match(/^\[([^\]]+)\]$/);
  if(h){flushPending();let n=cleanName(h[1].trim()),l=n.toLowerCase();
...
function currentSong(){const sections=appGetBase();const clean={};Object.keys(sections).forEach(k=>clean[cleanName(k)]=sections[k]);return{title:document.getElementById('songTitle')?.textContent?.trim()||'Imported Song',artist:document.getElementById('songArtist')?.textContent?.trim()||'Unknown Artist',key:document.getElementById('key')?.textContent?.trim()||'',sections:clean};}
...
function openSaved(i){const list=libraryList(),s=list[i];if(!s)return;appSetBase(s.sections||{});setMeta(s.title||'Untitled',s.artist||'Unknown Artist',s.key||'');appRender();localStorage.setItem(CURRENT_KEY,JSON.stringify(s));toast('Song opened');}
...
function install(){addControls();renderLibrary();window.__worshipOpenSaved=openSaved;window.__worshipDeleteSaved=deleteSaved;window.__worshipSaveSong=saveLocalSong;window.__worshipRenderLibrary=renderLibrary;const originalSave=window.saveLocal;window.saveLocal=function(show=true){saveLocalSong();};window.addEventListener('worshipchords:song-imported',e=>{const d=e.detail||{},u=d.sourceUrl||'',s=loadIntoEditor(d.text||'',{title:slugTitle(u),artist:artistFromUrl(u),sourceUrl:u});librarySave(s);addControls();});}

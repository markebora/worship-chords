...
function addControls(){const c=document.querySelector('#song .controls[style*="margin"]');if(!c||document.getElementById('songManagerExport'))return;const b=(id,label,fn)=>{const x=document.createElement('button');x.id=id;x.className='btn';x.type='button';x.textContent=label;x.onclick=fn;return x;};const input=document.createElement('input');input.id='songManagerFile';input.type='file';input.accept='.txt,text/plain';input.style.display='none';input.onchange=e=>{if(e.target.files[0])importTxt(e.target.files[0]);e.target.value='';};c.append(b('songManagerSave','💾 Save Song',saveLocalSong),b('songManagerExport','⬇ Export TXT',()=>exportSong()),b('songManagerImport','⬆ Import TXT',()=>input.click()),input);}
function install(){addControls();new MutationObserver(addControls).observe(document.body,{childList:true,subtree:true});window.addEventListener('worshipchords:song-imported',e=>{const d=e.detail||{},u=d.sourceUrl||'',s=loadIntoEditor(d.text||'',{title:slugTitle(u),artist:artistFromUrl(u),sourceUrl:u});librarySave(s);addControls();});}
